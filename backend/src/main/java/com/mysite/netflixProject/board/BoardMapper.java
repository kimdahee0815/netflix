...
public interface BoardMapper {
	...
	@Select("select board_num, member_id, board_title, board_content, "
			+ "to_char(board_date, 'YYYY-MM-DD') as board_date, board_reply from customercenter order by board_num desc")
	public List<BoardVO> getBoardList();

	@Insert("insert into customercenter (member_id, board_title, board_content) values "
	...
	@Select("SELECT board_num, member_id, board_title, board_content, "
			+ "to_char(board_date, 'MM\"월\" DD\"일\" HH24\"시\"') as board_date, board_reply FROM customercenter WHERE board_title LIKE CONCAT('%', #{search}, '%') OR board_content LIKE CONCAT('%', #{search}, '%') "
